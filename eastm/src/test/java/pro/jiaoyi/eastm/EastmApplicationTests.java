...
//        String code = "000002";
    ...
    public void dao() {
        ...
        List<EmCList> all = emClient.getClistDefaultSize(false);
        for (EmCList emCList : all) {
            List<EmDailyK> dailyKs = emClient.getDailyKs(emCList.getF12Code(), end, 5000, false);
            ...
            log.debug("last={}", dailyKs.get(last));
            int count = kLineRepo.countByCode(emCList.getF12Code());
            if (count > 0) {
    ...
    public void sub() {
        ...
        System.out.println(integers.subList(i - 5, i));
    }
