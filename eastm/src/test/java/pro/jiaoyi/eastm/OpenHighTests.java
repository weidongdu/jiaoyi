...
//        BigDecimal b1005 = new BigDecimal("1.005");
//        BigDecimal b105 = new BigDecimal("1.05");
//        List<EmCList> openHighList = list.stream().filter(
//                em -> em.getF17Open().compareTo(b1005.multiply(em.getF18Close())) > 0
//                        && em.getF17Open().compareTo(b105.multiply(em.getF18Close())) < 0
//                        && em.getF6Amt().compareTo(BDUtil.B500W) > 0).toList();

        List<OpenEmCListEntity> openHighList = openEmCListRepo.findOpenHigh();

        for (OpenEmCListEntity em : openHighList) {
            String code = em.getF12Code();
            String tradeDate = em.getTradeDate();

            List<EmDailyK> ks = emClient.getDailyKs(code, LocalDate.now(), 500, false);
            if (ks.size() < 100) continue;
            int last = - 1;
            for (int i = 0; i < ks.size(); i++) {
                if (tradeDate.equalsIgnoreCase(ks.get(i).getTradeDate())) {
                    log.info("hit trade date={}", em);
                    last = i;
                    break;
                }
            }
            ...
            HashMap<Integer, Integer> kHighMap = new HashMap<>();

            for (int i = 0; i < 5; i++) {
                EmDailyK k = ks.get(last - i);
                //向前找, 看看 哪天 比现在高
                kHighMap.put(i, last);
                ...
                for (int j = 1; j < ks.size(); j++) {
                    ...
                    EmDailyK pre = ks.get(last - i - j);
                    if (pre.getHigh().compareTo(k.getOpen()) > 0) {
                        kHighMap.put(i, j);
                        break;
                    }
                }
            ...
            //过滤出 key=0 是最大的 map
            if (kHighMap.get(0) > 60
                    && 2 > kHighMap.get(1)
                    && 2 > kHighMap.get(2)
                    && 2 > kHighMap.get(3)
                    && 2 > kHighMap.get(4)

                    && kHighMap.get(0) > kHighMap.get(1)
                    && kHighMap.get(0) > kHighMap.get(2)
                    && kHighMap.get(0) > kHighMap.get(3)
                    && kHighMap.get(0) > kHighMap.get(4)) {

                log.warn("{} {}", em.getF12Code() + em.getF14Name() + em.getTradeDate(), kHighMap);

                int start = Math.max(0, last - 70);
                BigDecimal day = emClient.amtTop10p(ks.subList(start, last));
                BigDecimal hour = day.divide(new BigDecimal(4), 2, RoundingMode.HALF_UP);
                BigDecimal f = hour.multiply(BDUtil.b0_1);

                BigDecimal fx = em.getF6Amt().divide(f, 4, RoundingMode.HALF_UP);
                //给出后面5天的 涨幅
                StringBuilder sb = new StringBuilder();
                sb.append("fx=").append(fx).append(" amt=").append(BDUtil.amtHuman(em.getF6Amt()));
                for (int i = 0; i <= 5; i++) {
                    int pi = last + i;
                    if (pi > ks.size() - 1) {
                        break;
                    }
                    EmDailyK k = ks.get(pi);
                    BigDecimal p = k.getClose().subtract(em.getF17Open()).divide(em.getF17Open(), 4, RoundingMode.HALF_UP);
//                    log.info("=>{} {}",i,BDUtil.p100(p));
                    sb.append(" ").append(i).append("=").append(BDUtil.p100(p));
                }
